...
//NI UNTUK YANG DAH LOGIN/AUTHENTICATED
...
// NI BELOM LOGGED IN/AUTHENTICATED


class UnauthenticatedProfileView extends StatefulWidget {
  const UnauthenticatedProfileView({super.key});

  @override
  State<UnauthenticatedProfileView> createState() => _UnauthenticatedProfileViewState();
}

class _UnauthenticatedProfileViewState extends State<UnauthenticatedProfileView> {
  @override
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (isWideScreen) {
          ...
          return Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
               Expanded(flex: 3, child: _MainLoginCard()),
               SizedBox(width: 20),
               Expanded(flex: 2, child: _AccountUsageCard()),
            ],
          );
        } else {
